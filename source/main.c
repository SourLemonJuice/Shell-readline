...
int main(int argc, char *argv[])
{
    if (argc >= 2) {
        ...
        fprintf(stderr, "Usage: shell-readline\n\n");
        fprintf(stderr, "The final user input will be printed to stdout, the readline interface will displayed on stderr.\n");
        fprintf(stderr, "In bash, use \"shell-readline | cat -\" to provide stdin to another command.\n");
        fprintf(stderr, "Also can use \"input=$(shell-readline)\" to write input to a variable.\n");
    ...
    if (input != NULL) {
        ...
        fprintf(stdout, "[shell-readline: Unknown error, input string is NULL]\n");
        exit(EXIT_FAILURE);
    }
